...
function initSparrow() {
    ...
    setAutoLookup = function 
    
    (id, data, basename, required, changeEvent, storeInstance, beforeSend, maxSelection, filter, selectionCallback, options, placeholder) {
            ...
            $(ms.input)
                ...
                .append(
                    '<input type="text" class="" id="' +
                        ...
                        '-required" name="' +
                        id +
                        '-required" required="required" data-msg-required=" " style="visibility:hidden; display:none;">'
                );
        }
        if (changeEvent) {
        ...
        $(ms).on('focus', function () {
            var setDataUrlParams = {
                bid: eval($('#h' + basename).val()),
                onfocus: true,
            };
            if (filter != null) {
            ...
            if (data['dataUrl']) {
                var openIframeDialog = openIframe('/b/iframe_index/' + data['dataUrl'], data['title'], function (dataObject) {
                    ...
                    if (
                        ...
                        basename != '' &&
                        (($('#h' + basename).val() != undefined && parseInt($('#h' + basename).val()) != dataObject[basename]) || $('#h' + basename).val() == undefined)
                    ) {
                        isUpdate = false;
                    }
    ...
    app.pager = function ($scope, $routeParams, showPager, modelName, appName, route) {
        ...
        if (showPager) {
            $('#top_action_bar').append(
                '<div class="btn-group btn-group-sm" style="float: right; margin-right: 5px; margin-top: 7px;">' +
                    '<a class="btn btn-default icon-arrow-1-left paginate_button pager-button" data-pager="previous"></a>' +
                    '<a class="btn btn-default icon-arrow-1-right paginate_button pager-button" data-pager="next"></a>' +
                    '</div>'
            );
    ...
    app.controller("YesNoController", [
        "$scope",
        "close",
          ...
          <div class="modal-content">\
            ...
            <div class="modal-footer">\
              <button type="button" style="' +
                hidePositiveBtn +
                '" ng-click="close(true)" class="btn btn-primary applyBtn" data-dismiss="modal">' +
                positiveBtnText +
                '</button>\
              <button type="button" style="' +
                hideNegativeBtn +
                '" ng-click="cancel()" class="btn btn-default" data-dismiss="modal">' +
                negativeBtnText +
                '</button> \
            </div>\
          </div>\
    ...
    app.global = {
        keys: {
            ...
            USERNAME: 'user_name',
            GOOGLE_ANALYTICS_CODE: 'google_analytic_code',
        },
        globelObj: {},
    ...
    function mainController($scope, $rootScope, $route, $compile, $location) {
        ...
        $scope['availableSearchParams98'] = [
            ...
            {
                key: 'po_nr',
                name: 'Purchase order / Inquiry',
            },
            {
                key: 'so_nr',
                name: 'Sales order / Inquiry',
            },
            {
                key: 'to_nr',
        ...
        $scope.$on('advanced-searchbox:leavedEditMode', function (event, searchParameter, tableIndex, cacheParam) {
            ...
            $rootScope.searchFor = searchParameter.name + ' ' + searchParameter.value;
        ...
        // "addEventListener" when modal is opened and we click back button of the browser then backdrop will be removed and modal will be closed.
        ...
        window.addEventListener(
            ...
            function () {
                ...
                // For Emptying search-bar on opening different modules or on url change
                if ($route.current && $route.current.originalPath != '/search') {
                    $('.advancedSearchBox').find('span.remove-all-icon').click();
                }
            },
            false
        );
        ...
        $scope.goBack = function () {
            ...
            var index = 2;
            if (window.location.hash.split('?')[1] == 'backhis=true') {
                index = 1;
            }
        ...
        $('body').on('click', 'a.show-tab', function (e) {
            ...
            $('.tab-search')
                .find('#' + currentId + '-search')
                .addClass('tab-search-active');
            ...
            $('#app_container').find('.tab-fav').removeClass('glyphicon-star').addClass('glyphicon-star-empty').css({
                color: '#797D83',
            });
            ...
            $('.fav_page').each(function () {
                if ($(this).css('display') == 'block') {
                    ...
                    if (current_page_url == url) {
                        $('#app_container').find('.tab-fav').removeClass('glyphicon-star-empty').addClass('glyphicon-star').css({
                            color: '#1174da',
                        });
            ...
            for (var i = 0; i < tabtables.length; i++) {
                ...
                $(tabtables[i]).find('tr').removeClass('selected-row-background');
            }
            ...
            // code preserve search box values in tab
            ...
            success: function (data) {
                $scope.$applyAsync(function () {
                    ...
                    if (data.code == 0) {
                        ...
                        if (dataMsg === '') {
                            dataMsg = data.error||'Error occurred.';
                        }
    ...
    app.run(function ($rootScope, $route, $templateCache, $location) {
        ...
        $rootScope.$on('$routeChangeSuccess', function () {
            if (app.inIframe() == false) {
                app.global.set(app.global.keys.CURRENT_ROUTE, $route.current);
                var addMode = false;
                if (app.getQueryStringParams('backhis') == 'true') {
                    addMode = true;
                }
                var locationHistory = JSON.parse(app.getStorage(app.global.keys.LOCATION_HISTORY)) || [];
                if (
                    locationHistory.length == 0 ||
                    (locationHistory.length > 0 && locationHistory[locationHistory.length - 1].Location != window.location.hash && $route.current && addMode == false)
                ) {
    ...
    app.setDatatable = function (callback, compile, DTOptionsBuilder, scope, rootScope, currentRoute, listing, listingall, ModalService) {
        ...
        scope.openColumnConfigDialog = function (event, tabIndex) {
            ...
            for (var a = 0; a < displayColumns.length; a++) {
                ...
                var configColumn =
                    '<div class="configColumn" column-name="' +
                    displayColumns[a].name +
                    '">\
                              <input type="checkbox" id="id_check_' +
                    displayColumns[a].name +
                    ...
                    '>\
                              <label for="id_check_' +
                    displayColumns[a].name +
                    '" ></label><span class="configColumnLabel">' +
                    currentColumns[a].title +
                    '</span>\
                            </div>';
        ...
        var options = DTOptionsBuilder.newOptions()
            .withOption('fnServerData', function (sSource, aoData, fnCallback) {
                ...
                if (listing.reOrder && $('.reOrder' + listing.uIndex).find('.up_down').length == 0) {
                    var reOrderBtn =
                        '<span style="float: left;margin-top: 1px;position:relative;" class="up_down"><i id="col_order" ng-click="openColumnConfigDialog($event,' +
                        listing.uIndex +
                        ');" style="font-size: 14px;cursor: pointer;" class="icon-settings" title="Configure columns"></i></span>';
            ...
            .withOption('drawCallback', function () {
                // Handle group by datatable view
                if (scope['searchParams' + listing.index] != undefined) {
                    var groupColumn = scope['searchParams' + listing.index]['group_by'];
                    if (groupColumn != undefined) {
                        var tableColumns = scope['dtColumns' + listing.index];
                        var gorupByColLength = tableColumns.length - 1;

                        var api = this.api();
                        for (i = 0; i < tableColumns.length; i++) {
                            if (tableColumns[i]['mData'] == groupColumn) {
                                groupColumnIndex = i;
                            }
                            var headerNodes = api.column(i).header();
                            $(headerNodes).removeClass('hidden');
                        }
                        ...
                        api.column(groupColumnIndex, { page: 'current' })
                            ...
                            .each(function (group, i) {
                                var headerNode = api.column(groupColumnIndex).header();
                                var cellNode = api.cell(i, groupColumnIndex).node();
                                $(headerNode).addClass('hidden');
                                $(cellNode).addClass('hidden');
                                if (last !== group) {
                                    $(rows)
                                        .eq(i)
                                        .before('<tr class="group"><td colspan="' + gorupByColLength + '">' + group + '</td></tr>');
                                    last = group;
                                }
                            });
                    }
    ...
    app.setDatePicker = function () {
        ...
        $('.datePicker').on('change', function (e) {
            ...
            $('.datePicker').closest('.form-group').find('.help-block').hide();
        });
    };
    ...
    function searchbarEventsHandler($scope, $route) {
        $scope.$on('advanced-searchbox:leavedEditMode', function (event, searchParameter, tableIndex, cacheParam) {
            ...
            if (searchParameter['type'] == 'datePicker') {
                ...
                $scope['searchParams' + tableIndex][searchParameter.key + '_from_date'] = para_val[0] + '00:00';
                $scope['searchParams' + tableIndex][searchParameter.key + '_to_date'] = para_val[1] + ' 23:59';
            }

            // if (searchParameter['type'] == 'group_by') {
            //     para_val = searchParameter.options;
            //     $scope['searchParams' + tableIndex][searchParameter.key] = Object.keys(para_val).find((key) => para_val[key] === searchParameter.value);
            // }
        ...
        $scope.$on('advanced-searchbox:removedAllSearchParam', function (event, tableIndex, cacheParam) {
            ...
            // Remove search filter when page is changes
            // if (cacheParam == 'true') {
    ...
    app.setup = function ($scope, $rootScope, $route, $compile, DTOptionsBuilder, DTColumnBuilder, $templateCache, config, ModalService) {
        ...
        if (topActionbar) {
            ...
            if (topActionbar.add) {
                $rootScope.onAdd = function (event) {
                    window.location =
                        topActionbar.add.url +
                        '0' +
                        (topActionbar.add.filters != undefined && topActionbar.add.filters != '' ? '?' + topActionbar.add.filters : '') +
                        '/?&backhis=true';
                };
            ...
            if (topActionbar.edit) {
                $rootScope.onEdit = function (event) {
                    ...
                    if ($scope['selected1'][0] == undefined) {
                        ...
                        app.redirect(
                            topActionbar.edit.url +
                                $scope['selected1'][0] +
                                (topActionbar.edit.filters != undefined && topActionbar.edit.filters != '' ? '?' + topActionbar.edit.filters : '')
                        );
                    }
                };
        ...
        if (allListing) {
            ...
            for (var i = 0; i < allListing.length; i++) {
                ...
                if (listing.crud) {
                    ...
                    idColumn = DTColumnBuilder.newColumn(null)
                        .withTitle(
                            '<input class="allListing' +
                                tIndex +
                                ' magic-checkbox"  id="' +
                                chkHeaderId +
                        ...
                        .renderWith(function (data, type, full, meta) {
                            var instanceId = $(meta.settings.nTable)
                                .attr('dt-instance')
                                .replace(/[^\d.]/g, '');
                            ...
                            return (
                                ...
                                chkId +
                                '"  class="magic-checkbox"  ng-checked="isSelected(' +
                                instanceId +
                                ', ' +
                                data.id +
                ...
                // if (idColumn == false) {
                //     idColumn = DTColumnBuilder.newColumn(null).notVisible();
                // }
                ...
                for (var k = 0; k < listing.columns.length; k++) {
                    ...
                    if (listColumn.link) {
                        ...
                        column.renderWith(function (data, type, full, meta) {
                            ...
                            if (dialog) {
                                ...
                                return (
                                    '<a class="link-iframe-item" ng-click="onEditLink(\'' +
                                    url +
                                    "','" +
                                    iframeTitle +
                                    ' - ' +
                    ...
                    if (listColumn.onEdit) {
                        ...
                        column.renderWith(function (data, type, full, meta) {
                            ...
                            return (
                                '<div class="mouseover-edit-col" id="' +
                                full.id +
                                '" name="' +
                                colData.name +
                                '" title="Edit item" instance="' +
                                instanceId +
                                '" col=' +
                                meta.col +
                                ' >\
                            ...
                            <input type="' +
                                type +
                                '" class="input-edit-link" id="input_' +
                                colData.name +
                                '_' +
                                full.id +
                                '" value="' +
                                data +
                                '">\
                            <i class="icon-pencil-1 cell-edit-icon" id="icon_' +
                                colData.name +
                                '_' +
                                full.id +
                                '"></i>\
        ...
        var height = $(window).height() - ($('.nav_menu').outerHeight() + 60);
        if (app.inIframe()) {
            height = height + 45;
        ...
        if ($('.details-footer').length != 0) {
            var totalHeight = height - ($('.details-footer').height() + 30);
            $('.details-body').css({
            ...
            for (i = 0; i < allListing.length; i++) {
                ...
                if (listing.inlineCrud) {
                    ...
                    function actionsHtml(data, type, full, meta) {
                        ...
                        if (isEdit) {
                            template +=
                                '<div class="pull-right" style="width: 65px;" id="' +
                                data.id +
                                '_inline_crud"><i class="icon-pencil-1 list-btn" ng-click="inlineEdit(' +
                                instanceId +
                                ',' +
                                data.id +
                                ')"></i>&nbsp;';
                    ...
                    $scope['dtColumns' + tIndex].push(
                        DTColumnBuilder.newColumn(null).withTitle('<span style="padding-right:65px;"></span>').withClass('text-right').notSortable().renderWith(actionsHtml)
                    );
    ...
    var openIframe = function (src, title, closeDialogCallback, hasDomain, tableIndex, closeIframeCallback, is_reload) {
        ...
        var iframeHTML =
            '<div id="' +
            iframeModelId +
            '"  class="modal fade frame-dialog" tabindex="-1" role="dialog" data-keyboard="false"  data-backdrop="static">\
    ...
    app.applyEditMode = function (containerId, firstElementId) {
        ...
        $(containerId + ' label')
            .not(preDisabledLabels)
            .removeClass('read-only-labels');
        ...
        $(containerId + ' .ms-trigger')
            .filter(function () {
                return $(this)
                    .parent()
                    .is(':not(' + preDisabledMsIds + ')');
            })
            .show();
    ...
    app.getEntityIframeURL = function (appName, modelName, entityId, related_to, type) {
        ...
        if (appName == 'sales') {
            ...
            if (modelName == 'maintenanceschedule') {
                url = '/b/iframe_index/#/' + appName + '/' + 'schedule' + '/' + entityId;
            }
            ...
            if (modelName == 'deal') {
                url = '/b/iframe_index/#/crm/' + modelName + '/' + entityId;
            }
