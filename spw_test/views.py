    ...
    try:
        ...
        for emp in all_employee:
            response["data"].append(
                {"id": emp.id, "first_name": emp.first_name, "last_name": emp.last_name, "mobile_num": emp.mobile_num, "designation": emp.designation.designation}
            )
...
def employee_address(request, address_id):
    try:
        ...
        return render(request, "spw_test/employee_address.html", {"address": address_obj, "employees": employees, "selected_employee_id": selected_employee_id})
...
def salary_slip(request, id):
    try:
        ...
        leaves = EmployeeLeaveMaster.objects.filter(employee=employee, leave_status="Approved", leave_start_date__year=month_date.year, leave_start_date__month=month_date.month)

        leaves_taken = sum(float(leave.duration) for leave in leaves)
        lwp_days = sum(float(leave.duration) for leave in leaves if leave.leave_type.leave_name == "LWP")
        ...
        gross_earnings = basic + hra + employee.special_allowance + employee.other_allowance

        total_deductions = pf + lwp_deduction + employee.other_deduction + 200
        ...
        context = {
            ...
            "net_salary": f"{round(net_salary, 2):.2f}",
...
def salary_slip_download(request):
    try:
        if request.method == "POST":
            ...
            if not payload_raw:
                return HttpResponse(AppResponse.get({"code": 0, "msg": "Payload missing"}), content_type="application/json", status=400)
            ...
            html_string = render_to_string("spw_test/salary_slip_download.html", payload)

            config = pdfkit.configuration(wkhtmltopdf=r"C:\Program Files\wkhtmltopdf\bin\wkhtmltopdf.exe")

            options = {"page-size": "A4", "encoding": "UTF-8", "enable-local-file-access": "", "quiet": ""}
        ...
        return HttpResponse(AppResponse.get({"code": 0, "error": "An error occurred while processing", "msg": e}), content_type="json")
